...
export function ConferenciaListaScreen({ navigation }: Props) {
  ...
  const load = useCallback(async () => {
    setError(null)
    try {
      const board = await loadConferenciaBoardUseCase()
      setPedidos(board.pedidos)
  ...
  const atribuirTarefa = async (nutarefa: number) => {
    try {
      ...
      if (isDomainError(e) && e.code === 'WMS_TASK_ASSIGN_USER_REQUIRED') {
        showWmsError('Conferência', e, 'Não foi possível identificar o usuário.')
        return
      }
