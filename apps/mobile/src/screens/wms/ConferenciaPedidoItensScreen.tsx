...
import { getConferenciaItensPedido, postConferenciaTarefa } from '../../services/wmsApi'
import type { ItemTarefaWms } from '../../types/wms'

type Props = NativeStackScreenProps<HomeStackParamList, 'ConferenciaPedidoItens'>
...
export function ConferenciaPedidoItensScreen({ navigation, route }: Props) {
  ...
  const load = useCallback(async () => {
    setError(null)
    try {
      const data = await getConferenciaItensPedido(nunota)
      setItems(data)
    } catch (e) {
  ...
  const iniciar = async () => {
    setCriando(true)
    try {
      const res = await postConferenciaTarefa({ nunota, codemp })
      const msg = res.existente ? 'Já existia tarefa CON — abrindo.' : 'Tarefa criada.'
      showWmsSuccess('Conferência', msg, () =>
        navigation.replace('ConferenciaTarefaItens', {
      ...
      showWmsError('Conferência', e, 'Erro ao criar tarefa')
    } finally {
      setCriando(false)
    }
  }
